...
Muldis::DBP::API -
Virtual Database Management System for Perl 6

=head1 VERSION

This document is Muldis::DBP::API version 0.1.0.-9 for Perl 6.
...
DBP> (B<MDBP>) library API for Perl 6, authority C<http://muldis.com>,
...
concrete Perl 6 library named C<Muldis::DBP::API>; however,
no Perl 6 library with that name actually exists, and it never should
exist; thus C<Muldis::DBP::API> is a I<virtual> library.

An actual concrete Perl 6 library that implements and is fully conformant
...
An I<object> (unqualified) is a Perl 6 object; this concept does not exist
inside a DBMS virtual machine in the general case spoken to by this
document; this term is only used outside the DBMS, in regular Perl 6 space.

=head2 class

A I<class> (unqualified) is a Perl 6 class; this concept does not exist
inside a DBMS virtual machine in the general case spoken to by this
document; this term is only used outside the DBMS, in regular Perl 6 space.

=head1 TYPES

The B<Muldis DBP> (B<MDBP>) library API for Perl 6 formally
...
passing to/from the main Perl 6 environment must be losslessly represented
...
Perl 6 has a direct C<Any> analogy which is provided by the Perl 6
class C<Any>.
...
Perl 6 has a direct C<Boolean> analogy which is provided by the Perl 6
enumeration C<Bool>.
...
Perl 6 has a direct C<Integer> analogy which is provided by the Perl 6
class C<Int>.
...
Perl 6 has a direct C<Fraction> analogy which is provided by the Perl 6
class C<FatRat>.
...
Perl 6 has a direct C<String> analogy which is provided by the Perl 6 role
class C<Blob>, or specifically by a composing class that uses a list of
unlimited-magnitude integers, an immutable list being the best match.
...
Perl 6 has a direct C<Bits> analogy which is provided by the Perl 6 role
class C<Blob>, or specifically by a composing class that uses a list of
...
Perl 6 has a direct C<Blob> analogy which is provided by the Perl 6 role
class C<Blob>, or specifically by a composing class that uses a list of
...
Perl 6 has a direct C<Text> analogy which is provided by the Perl 6
class C<Str>.
...
Perl 6 has a direct C<Array> analogy which is provided by the Perl 6
class C<List>.
...
Perl 6 has a direct C<Set> analogy which is provided by the Perl 6
class C<Set>.
...
Perl 6 has a direct C<Bag> analogy which is provided by the Perl 6
class C<Bag>.
...
Perl 6 has a direct C<Tuple> analogy which is provided by the Perl 6
class C<Capture>.  This analogy extends to the fact that a primary purpose
...
Perl 6 has a direct C<Interval> analogy which is provided by the Perl 6
class C<Range>.
...
Perl 6 has a direct C<Variable> analogy which is provided by the Perl 6
class C<Scalar> in particular, but also the Perl 6 I<container> concept in
general, of which the Perl 6 class C<Proxy> is another example.
...
An C<External> value can in general only exist in the memory of the Perl 6
...
Perl 6 has a direct C<Excuse> analogy which is provided by the Perl 6 class
C<Failure>.

Also of note, C<X::AdHoc> is the Perl 6 type into which objects are wrapped
if they are thrown as exceptions, but don't inherit from C<Exception>.
...
Perl 6 has a direct C<Excuse_No_Reason> analogy which is provided by the
Perl 6 class C<Nil>.
...
Perl 6 has a direct C<Excuse_No_Such_Ord_Pos> analogy which is provided by
the Perl 6 class C<X::OutOfRange>.
...
As of this time, any stable version of Perl 6 at all (language version
C<6.c> from 2015 and up) is expected to be able to provide the necessary
foundation for Perl 6 code that is able to speak the B<Muldis DBP>
API.  If this turns out not to be the case, the minimum required Perl
6 version can be raised.
